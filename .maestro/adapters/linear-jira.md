...
3. **Cache state IDs:** for Linear, persist the `name → { id, type }` mapping (keyed by native state name) somewhere accessible (e.g. `config.backend.stateCache`) so `set_status` lookups are fast and do not require a network call per transition. For Jira, cache transition IDs per issue-type similarly.
