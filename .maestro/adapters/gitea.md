...
Variable conventions: `<owner>`/`<repo>` come from splitting `config.backend.repo` (format `owner/name`) on `/`; `<id>`/`<ref>` is the issue number parsed from the item id; `<token>` is `config.backend.token` or `$GITEA_TOKEN`.
