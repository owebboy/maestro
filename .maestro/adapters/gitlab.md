...
Set a specific subtask's state marker in the description's Markdown task list. State encoding: todo = `[ ]`, doing = `[~]`, done = `[x]` (matching the files/gitea adapters; GitLab renders `[~]` as literal text rather than a checkbox, but it round-trips through `get_item`).
...
mcp__gitlab__create_issue_note(
  project_id: <config.backend.project_id>,
  issue_iid: <ref>,
  body: "artifact: <kind> <url_or_path>"
)
```

**CLI:**
```bash
glab issue note <ref> -m "artifact: <kind> <url_or_path>"
```
...
Record a relationship between two issues. GitLab has no native typed relations in this adapter; Maestro posts a structured note for every `kind`, and for `duplicate` additionally labels and closes the source issue.

- Any `kind` (`blocks`, `blocked-by`, `relates-to`, `duplicate`): post a note `"<kind> #<target>"` on the source issue.
- `duplicate` only: also apply the `duplicate` label and close the issue.
...
**CLI:**
```bash
glab issue note <ref> -m "<kind> #<target>"
# if kind == duplicate:
